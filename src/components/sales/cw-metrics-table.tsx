...
export function CWMetricsTable({
  channelId,
  refreshKey,
}: {
  channelId: string;
  ...
  const { data, isLoading: loading } = useQuery<{
    ...
    queryKey: ['coupang-metrics', channelId, page, search, refreshKey],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/coupang-metrics?${params}`);
      return res.json();
    },
    enabled: !!channelId,
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-semibold">판매 내역 (쿠팡 윙)</h2>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-center">방문자</TableHead>
