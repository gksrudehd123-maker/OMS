...
- **사이드바 메뉴 정리**: 채널 분석/사용자 관리/감사 로그를 「설정 / 관리」 드롭다운으로 그룹화하여 메인 메뉴 간소화
- **API 동기화 조회 기간 제한**: 최대 7일 제한 (UI 경고 + 서버 검증), 하루 단위 분할 호출 + 진행률 표시 (n/n일)
- **광고 손익분기 키워드 자동 조회**: 탭 진입 시 오늘 미조회 키워드 자동 감지 → 순차 조회 + 진행률 표시 (n/n개), 완료 후 데이터 자동 갱신
- **로켓그로스 엑셀 업로드 개선**: 파일명에서 판매 날짜 자동 추출 + 확인 팝업 (수동 날짜 선택 제거), 판매가 자동 계산 (salesAmount÷salesQuantity), 신규 상품 팝업에 판매가 입력 칸 추가, fulfillmentFee/couponDiscount 기본값 0
