...
- **CS 관리**: A/S 접수/처리, 상태별 필터, 월별/상태별 그룹 뷰, 상품 정보 카탈로그, 메시지 템플릿
- **문자 발송**: 알리고 SMS 연동, 개별/일괄 발송, `{{변수}}` 템플릿, 실시간 바이트/SMS/LMS 미리보기, 발송 이력 + 재발송
- **인증/권한**: 로그인 + RBAC (OWNER/MANAGER/STAFF) + 채널별 접근 제한
- **감사 로그**: 데이터 변경 이력 추적 (생성/수정/삭제/업로드/동기화/SMS 발송)
...
#### 문자 발송 기능 (알리고 SMS API) ✅ 구현 완료 (2026-04-13)

**인프라**

- [x] Prisma `SmsLog` 모델 (수신자/발신자/본문/타입/상태/결과코드/테스트여부/발송자)
- [x] `MessageTemplate.channel` 필드 (SMS/LMS/KAKAO)
- [x] `src/lib/aligo.ts` — `sendSms()`, `getRemainingQuota()`, 바이트 기반 SMS/LMS 자동 판별, `testmode_yn` 파라미터로 테스트 모드 지원
- [x] 환경변수: `ALIGO_API_KEY`, `ALIGO_USER_ID`, `ALIGO_SENDER`, `ALIGO_TEST_MODE`

**API 라우트**

- [x] `POST /api/sms/send` — 발송 (감사 로그 기록, `SmsLog` 저장)
- [x] `GET /api/sms/logs` — 이력 조회 (페이징, 전화번호/상태 필터)
- [x] `GET /api/sms/quota` — 잔여 건수 (SMS/LMS/MMS)

**UI (CS 페이지)**

- [x] 메시지 템플릿 관리 — 채널(SMS/LMS/KAKAO) 선택 + 채널 배지 표시, `{{변수명}}` 지원
- [x] CS 목록 행 "문자" 아이콘 버튼 — 빠른 개별 발송
- [x] CS 수정 다이얼로그 전화번호 옆 "문자" 버튼
- [x] 발송 모달 — 템플릿 선택/직접 입력, 변수 입력, 실시간 미리보기(바이트/SMS/LMS 배지), LMS 제목
- [x] **일괄 문자 발송** — 상단 버튼 → 수신자 체크(완료건 기본 제외), 템플릿/본문, `{{고객명}}`·`{{제품명}}` 자동 치환, 공용 변수, 순차 발송 진행률
- [x] 발송 이력 탭 (4번째 탭) — 필터, 잔여 건수 배지, **재발송 버튼**

**알리고 계정 연동**

- [x] 회원가입 + 본인인증 + 발신번호 승인 + API Key 발급 + IP 화이트리스트
- [ ] 충전 (SMS 약 8.4원/건, LMS 약 25원/건, 최소 1만원)

**향후 2단계: 업무폰 Android 앱 (수신 감지)**

- 알리고는 발송 전용이라 답장 수신 불가 → 발신번호 등록된 업무폰에 사이드로딩 APK로 문자 수신 감지 → OMS 서버 전송 → CS 대화 통합 관리
- React Native 고려. 별도 프로젝트로 진행.
...
| 04-13 | SMS      | 알리고 SMS 연동 — 발송/이력/잔여 조회 API, 감사 로그, SMS/LMS 자동 판별                 |
| 04-13 | SMS      | 발송 UI — 템플릿 채널(SMS/LMS/KAKAO), 행/편집 "문자" 버튼, 미리보기(바이트/타입 배지)   |
| 04-13 | SMS      | 일괄 문자 발송 — 수신자 체크(완료건 기본 제외), `{{고객명}}`·`{{제품명}}` 자동 치환     |
| 04-13 | SMS      | 발송 이력 탭 — 전화번호/상태 필터, 잔여 건수 배지, 재발송 버튼                          |
