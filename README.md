...
> 운영 안정성과 데이터 정합성을 높이기 위한 항목들.

#### 완료
- [x] 전일/전주/전월 비교 — KPI 카드에 이전 기간 대비 증감률(▲▼%) 표시
- [x] ROAS 분석 — 채널별 광고비 대비 매출(광고 수익률) 계산, 차트 툴팁에 표시
- [x] 업로드 변경 사항 요약 — 업로드 후 전체/성공/중복/신규상품 건수 표시

#### 데이터 정합성
- [x] 주문 중복 업로드 방지 — Prisma unique constraint + P2002 에러 처리로 중복 건 skip
- [x] .env.example 업데이트 — 네이버 검색 API 키 등 누락된 환경변수 추가

#### 운영 안정성
- [x] 감사 로그 (Audit Log) — 상품/채널/사용자/광고비/설정/업로드 CUD 이력 기록 + 조회 페이지
- [ ] 핵심 로직 테스트 — 마진 계산, 엑셀 파서, 수수료 계산 단위 테스트
